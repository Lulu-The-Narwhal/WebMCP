// Browser copy of functions/_lib/mock-flights.ts's generateMockFlights --
// duplicated on purpose (see sponsored-slot-client.js's comment); covered
// by test/mock-flights.test.ts on the TS side.
const AIRLINES = [
  ["DL", "Delta"], ["UA", "United"], ["AA", "American"], ["BA", "British Airways"],
  ["LH", "Lufthansa"], ["AF", "Air France"], ["EK", "Emirates"], ["QR", "Qatar Airways"],
];

async function seededBytes(str) {
  const data = new TextEncoder().encode(str);
  const digest = await crypto.subtle.digest("SHA-256", data);
  return new Uint8Array(digest);
}

export async function generateMockFlights(origin, destination, date) {
  ...
  for (let i = 0; i < numFlights; i++) {
    const slice = seed.slice(i * 4, i * 4 + 4);
    const bytes = slice.length === 4 ? slice : seed.slice(0, 4);
